...
AlarmID_t TimeAlarmsClass::triggerOnce(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext){   // trigger once at the given time_t
     if( value > 0)
		 return create(value, onTickHandler, pTickHandlerContext, IS_ONESHOT, dtExplicitAlarm);
     else
...
AlarmID_t TimeAlarmsClass::alarmOnce(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext){   // trigger once at the given time of day
     if( value <= SECS_PER_DAY)
		 return create(value, onTickHandler, pTickHandlerContext, IS_ONESHOT, dtDailyAlarm);
     else
        return dtINVALID_ALARM_ID; // dont't allocate if the time is greater than one day 	  
}

AlarmID_t TimeAlarmsClass::alarmOnce(const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext){   // as above with HMS arguments
	return create(AlarmHMS(H, M, S), onTickHandler, pTickHandlerContext, IS_ONESHOT, dtDailyAlarm);
}

	AlarmID_t TimeAlarmsClass::alarmOnce(const timeDayOfWeek_t DOW, const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext){  // as above, with day of week 
	return create((DOW - 1) * SECS_PER_DAY + AlarmHMS(H, M, S), onTickHandler, pTickHandlerContext, IS_ONESHOT, dtWeeklyAlarm);
}
   
	// this method will now return an error if the value is greater than one day - use DOW methods for weekly alarms   
	AlarmID_t TimeAlarmsClass::alarmRepeat(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext){ // trigger daily at the given time
    if( value <= SECS_PER_DAY)
		return create(value, onTickHandler, pTickHandlerContext, IS_REPEAT, dtDailyAlarm);
    else
       return dtINVALID_ALARM_ID; // dont't allocate if the time is greater than one day 	  
    }
    
	AlarmID_t TimeAlarmsClass::alarmRepeat(const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext){ // as above with HMS arguments
	return create(AlarmHMS(H, M, S), onTickHandler, pTickHandlerContext, IS_REPEAT, dtDailyAlarm);
    }
    
	AlarmID_t TimeAlarmsClass::alarmRepeat(const timeDayOfWeek_t DOW, const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext){  // as above, with day of week 
	return create((DOW - 1) * SECS_PER_DAY + AlarmHMS(H, M, S), onTickHandler, pTickHandlerContext, IS_REPEAT, dtWeeklyAlarm);
    }
      
	AlarmID_t TimeAlarmsClass::timerOnce(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext){   // trigger once after the given number of seconds 
		return create(value, onTickHandler, pTickHandlerContext, IS_ONESHOT, dtTimer);
    }
    
	AlarmID_t TimeAlarmsClass::timerOnce(const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext){   // As above with HMS arguments
		return create(AlarmHMS(H, M, S), onTickHandler, pTickHandlerContext, IS_ONESHOT, dtTimer);
    }
      
	AlarmID_t TimeAlarmsClass::timerRepeat(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext){ // trigger after the given number of seconds continuously
		return create(value, onTickHandler, pTickHandlerContext, IS_REPEAT, dtTimer);
    }
    
	AlarmID_t TimeAlarmsClass::timerRepeat(const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext){ // trigger after the given number of seconds continuously
		return create(AlarmHMS(H, M, S), onTickHandler, pTickHandlerContext, IS_REPEAT, dtTimer);
    }
    ...
    void TimeAlarmsClass::free(AlarmID_t ID)
    {
      if(isAllocated(ID))
      {
		...
		Alarm[ID].pTickHandlerContext = 0;
    	Alarm[ID].value = 0;
    ...
    // attempt to create an alarm and return true if successful
	 AlarmID_t TimeAlarmsClass::create(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext, uint8_t isOneShot, dtAlarmPeriod_t alarmType, uint8_t isEnabled)
    {
      ...
      {  
    	for(uint8_t id = 0; id < dtNBR_ALARMS; id++)
        {
    	  ...
    	  // here if there is an Alarm id that is not allocated
			...
			Alarm[id].pTickHandlerContext = pTickHandlerContext;
