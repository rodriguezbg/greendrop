...
private:
   ...
   AlarmID_t create( time_t value, OnTick_t onTickHandler, void *pTickHandlerContext, uint8_t isOneShot, dtAlarmPeriod_t alarmType, uint8_t isEnabled=true);
   
public:
  TimeAlarmsClass();
  // functions to create alarms and timers

  AlarmID_t triggerOnce(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext);   // trigger once at the given time_t

  AlarmID_t alarmRepeat(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext);                    // trigger daily at given time of day
  AlarmID_t alarmRepeat(const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext); // as above, with hms arguments
  AlarmID_t alarmRepeat(const timeDayOfWeek_t DOW, const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext); // as above, with day of week 
 
  AlarmID_t alarmOnce(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext);                     // trigger once at given time of day
  AlarmID_t alarmOnce(const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext);  // as above, with hms arguments
  AlarmID_t alarmOnce(const timeDayOfWeek_t DOW, const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext); // as above, with day of week 
  
  AlarmID_t timerOnce(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext);   // trigger once after the given number of seconds 
  AlarmID_t timerOnce(const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext);   // As above with HMS arguments
  
  AlarmID_t timerRepeat(time_t value, OnTick_t onTickHandler, void *pTickHandlerContext); // trigger after the given number of seconds continuously
  AlarmID_t timerRepeat(const int H, const int M, const int S, OnTick_t onTickHandler, void *pTickHandlerContext);   // As above with HMS arguments
